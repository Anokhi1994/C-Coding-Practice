...
#include <memory>
#include <sstream>
class Northeria : public Country{
private:
...
public:
	...
	Northeria(const std::string& sex_type, const int DD, const int MM, const int YYYY):
	...
	std::stringstream& generate_number(std::string sex_type,
		...
		std::stringstream s;
		if(sex_type == "F"){
			s << 1 << std::to_string(DD) << std::to_string(MM) << std::to_string(YYYY);

		}
		else if(sex_type == "M"){
			s << 2 << std::to_string(DD) << std::to_string(MM) << std::to_string(YYYY);
		}
